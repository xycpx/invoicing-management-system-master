layui.use(['laydate','table','layer'],function(){
    ...
    //头工具栏事件
    table.on('toolbar(overflows)', function(obj){
        switch(obj.event){
            case "add":
                openGoodsDialog();
                break;
     ...
     * 行监听
     */
    table.on("tool(overflows)", function(obj){
    ...
    form.on("submit(addOverflowList)", function (data) {
        var index = top.layer.msg('数据提交中，请稍候', {icon: 16, time: false, shade: 0.8});
        $.post(ctx + "/overflow/save", data.field, function (res) {
            if (res.code == 200) {
                setTimeout(function () {
                    top.layer.close(index);
                    top.layer.msg("操作成功！");
                   ...
                   window.location.href=ctx+"/overflow/index";
                }, 500);
            } else {
                layer.msg(
                    res.message, {
...
var datas=[];
 ...
 function reloadTableData(){
     layui.table.reload("overflowListTable",{
         data:datas
     })
     var total=0;
     for (let i = 0; i < datas.length; i++) {
